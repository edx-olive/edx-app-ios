...
extension OEXStyles {
    ...
    @objc var navigationTitleTextStyle : OEXTextStyle {
        return OEXTextStyle(weight: .semiBold, size: .base, color : navigationItemTintColor())
    }
    
    @objc var navigationButtonTextStyle : OEXTextStyle {
    ...
    @objc public func applyGlobalAppearance() {
        ...
        let textField = UITextField.appearance(whenContainedInInstancesOf: [UISearchBar.classForCoder() as! UIAppearanceContainer.Type])
        var defaultTextAttributes = [String: Any]()
        for (key, value) in searchBarTextStyle.attributes {
    ...
    @objc public var progressBarTintColor : UIColor {
        return UIColor(red: CGFloat(126.0/255.0), green: CGFloat(199.0/255.0), blue: CGFloat(143.0/255.0), alpha: CGFloat(1.00))
    }
    
    @objc public var progressBarTrackTintColor : UIColor {
        return OEXStyles.shared().neutralLight()
    }

    @objc var standardTextViewInsets : UIEdgeInsets {
        return UIEdgeInsetsMake(8, 8, 8, 8)
    }
    
    @objc var standardFooterHeight : CGFloat {
        return 50
    }
    ...
    @objc var discussionsBackgroundColor : UIColor {
        return OEXStyles.shared().neutralXLight()
    }

// Standard text Styles
    
    @objc var textAreaBodyStyle : OEXTextStyle {
        let style = OEXMutableTextStyle(weight: .normal, size: .small, color: OEXStyles.shared().neutralDark())
        style.lineBreakMode = .byWordWrapping
        return style
    }
    
    @objc func textFieldStyle(with size: OEXTextSize, color: UIColor = OEXStyles.shared().neutralDark()) -> OEXTextStyle {
...
// Standard button styles
    ...
    @objc func filledButtonStyle(color: UIColor) -> ButtonStyle {
    ...
    @objc var linkButtonStyle: ButtonStyle {
    ...
    @objc var filledEmphasisButtonStyle : ButtonStyle {
...
extension UISearchBar {
    @objc func applyStandardStyles(withPlaceholder placeholder : String? = nil) {
