...
class EnrollmentWebviewConfig : NSObject {
    @objc let searchURL: NSURL?
    @objc let exploreSubjectsURL: NSURL?
    @objc let courseInfoURLTemplate: String?
    @objc let nativeSearchBarEnabled: Bool
    @objc let subjectDiscoveryEnabled: Bool
...
class EnrollmentConfig : NSObject {
    ...
    @objc  let webviewConfig: EnrollmentWebviewConfig
    ...
    @objc func isCourseDiscoveryNative() -> Bool {
        return type == .Native
    }
}
...
extension OEXConfig {
    @objc var courseEnrollmentConfig : EnrollmentConfig {
